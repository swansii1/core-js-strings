...
function isPalindrome(str) {
  ...
  for (let i = 0; i < lowercaseStr.length; i += 1) {
    ...
    if (char >= 'a' && char <= 'z') {
      cleanedStr.push(char);
    }
  }

  const reversedStr = cleanedStr.slice().reverse();
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < splt.length; i += 1) {
    if (splt[i].length > lengthString.length) lengthString = splt[i];
  }
  return lengthString;
}
...
function reverseWords(str) {
  ...
  const reversWord = splt.map((word) => word.split('').reverse().join(''));
...
function extractNameFromTemplate(value) {
  ...
  const delOne = splt.slice(1);

  const lastPart = delOne[delOne.length - 1].replace('!', '');
  delOne[delOne.length - 1] = lastPart;
  return delOne.join(' ');
}
